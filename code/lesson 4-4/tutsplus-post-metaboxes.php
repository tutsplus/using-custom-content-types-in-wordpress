...
/*************************************************************
Metabox for differentiation
*************************************************************/
// function to create metabox
function tutsplus_differentiation_metabox() {
	
	add_meta_box(
		'tutsplus_differentiation_metabox', 
		__( 'Differentiation', 'tutsplus' ), 
		'tutsplus_differentiation_metabox_callback', 
		'moon', 
		'normal',
		'high', 
		''	
...
add_action( 'add_meta_boxes', 'tutsplus_differentiation_metabox' );

// metabox callback function
function tutsplus_differentiation_metabox_callback( $post ) {
	
	//nonce 
	wp_nonce_field( 'tutsplus_metabox_nonce_action', 'tutsplus_differentiation_nonce_field' );
	
	//field and label
	$value = get_post_meta( $post->ID, 'tutsplus_differentiation', true );
	echo '<label for="tutsplus_moon_differentiation">';
	_e( 'Is the moon differentiated (i.e. is its core, mantle and/or crust separate)?', 'tutsplus' );
	echo '</label> ';
	echo '<input type="checkbox" id="tutsplus_moon_differentiation" name="tutsplus_moon_differentiation"' . $value . ' />';

}

// save metadata
function tutsplus_save_differentiation_meta( $post_id ) {

	//nonce check
	if ( ! isset ( $_POST[ 'tutsplus_differentiation_nonce_field' ]) || ! wp_verify_nonce ( $_POST[ 'tutsplus_differentiation_nonce_field' ], 'tutsplus_metabox_nonce_action' )) {
		return;
	}
	...
	//save data
	if ( isset ( $_POST [ 'tutsplus_moon_differentiation' ] ) ) {
		update_post_meta( $post_id, 'tutsplus_differentiation', 'checked' );
	}
	else {
		update_post_meta( $post_id, 'tutsplus_differentiation', '' );
		
	}
	
}
add_action( 'save_post', 'tutsplus_save_differentiation_meta' );